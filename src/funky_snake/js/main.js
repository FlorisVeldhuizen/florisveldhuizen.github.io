...
const sketch = (p) => {
  ...
  */
  ...
  const drawGrid = () => {
    ...
    for (let i = 0; i < gridResX; i++) { // x-axis
      for (let j = 0; j < gridResY; j++) { // y-axis
        p.rect(i * squareSize + fieldOffsetX, j * squareSize + fieldOffsetY, squareSize, squareSize);
      }
    }
  }
  ...
  const drawScore = () => {
    ...
    // Highscore
    p.textAlign(p.RIGHT);
    p.text(`${String.fromCodePoint(0x1F3C6)}: ${highScore}`, fieldOffsetX + fieldWidth, fieldOffsetY - fontSize);
    // Current score
    p.textAlign(p.LEFT);
    p.text(`${String.fromCodePoint(0x1F36C)}: ${curScore}`, fieldOffsetX, fieldOffsetY - fontSize);
  }
  ...
  const drawSnacc = () => {
    p.textSize(32);
    p.text(`${String.fromCodePoint(0x1F36C)}`, snaccPos[0] * squareSize + fieldOffsetX, (snaccPos[1] + 0.9) * squareSize + fieldOffsetY);
  }
  ...
  const drawPlayer = () => {
    ...
    bodyParts.forEach((part,index) => {
      ...
      p.rect(part[0] * squareSize + fieldOffsetX, part[1] * squareSize + fieldOffsetY, squareSize, squareSize);
    });
  }
  ...
  const calcTouch = () => {
    ...
    let radianDistance = Math.atan2(p.mouseY-startTouchY, p.mouseX-startTouchX);
    if (radianDistance < 0) radianDistance = radianDistance + p.TWO_PI;
    if(distSquared(startTouchX, startTouchY, p.mouseX, p.mouseY) > 100) {
      if      (radianDistance > p.TWO_PI - p.QUARTER_PI || radianDistance < p.QUARTER_PI)          { setDirection(_RIGHT)}
      else if (radianDistance > p.QUARTER_PI && radianDistance < p.HALF_PI + p.QUARTER_PI)         { setDirection(_DOWN) }
      else if (radianDistance > p.HALF_PI + p.QUARTER_PI && radianDistance < p.PI + p.QUARTER_PI) { setDirection(_LEFT) }
      else if (radianDistance > p.PI + p.QUARTER_PI && radianDistance < p.TWO_PI - p.QUARTER_PI)  { setDirection(_UP)   }
    } else {
      touchDir = NIL;
    }
  }
  ...
  const drawTouch = () => {
    const bottomArc = () => p.arc(startTouchX, startTouchY, arcSize, arcSize, p.QUARTER_PI, p.HALF_PI + p.QUARTER_PI);
    const leftArc   = () => p.arc(startTouchX, startTouchY, arcSize, arcSize, p.HALF_PI + p.QUARTER_PI, p.PI + p.QUARTER_PI);
    const topArc    = () => p.arc(startTouchX, startTouchY, arcSize, arcSize, p.PI + p.QUARTER_PI, p.TWO_PI - p.QUARTER_PI);
    const rightArc  = () => p.arc(startTouchX, startTouchY, arcSize, arcSize, p.TWO_PI - p.QUARTER_PI, p.QUARTER_PI);
  ...
  p.keyPressed = () => {
    handleKeys(p.keyCode, p.UP_ARROW, p.DOWN_ARROW, p.LEFT_ARROW, p.RIGHT_ARROW);
  }
